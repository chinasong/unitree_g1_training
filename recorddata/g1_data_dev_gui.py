...
class ArmDevGUI(QWidget):
    def __init__(self):
        ...
        # ✅ 新增：全身录制按钮
        self.recording_flags["full_body"] = False
        self.record_data["full_body"] = []

        self.full_body_button = QPushButton("Start Full Body Recording")
        self.full_body_button.setCheckable(True)
        self.full_body_button.clicked.connect(partial(self.toggle_record_full_body, self.full_body_button))
        self.layout.addWidget(self.full_body_button, 2, 0, 1, 3)  # 占据底部三列
    ...
    def refresh(self):
        ...
        for key, flag in self.recording_flags.items():
            if flag and key != "full_body":  # 保持原逻辑不变
                row = [now]
        ...
        # ✅ 新增：全身关节录制（0~28）
        if self.recording_flags.get("full_body", False):
            row = [now]
            for idx in range(29):
                m = self.state.motor_state[idx]
                row += [m.q, m.dq, m.tau_est]
            self.record_data["full_body"].append(row)
    ...
    def toggle_record_full_body(self, button: QPushButton):
        key = "full_body"
        if not self.recording_flags[key]:
            self.record_data[key].clear()
            self.recording_flags[key] = True
            button.setText("Stop Full Body Recording")
        else:
            self.recording_flags[key] = False
            button.setText("Start Full Body Recording")
            self.save_csv_full_body()
    ...
    def save_csv_full_body(self):
        now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"g1_full_body_{now}.csv"
        ...
        print(f"✅ Saved {filename} with {len(self.record_data['full_body'])} frames.")
